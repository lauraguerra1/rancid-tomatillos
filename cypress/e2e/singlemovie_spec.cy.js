describe('single movie page', () => {
  ...
  it('should be able to click back and forth', () => {
    stubSingleFetch(436270, 404, blackAdam)
    stubSingleFetch('436270/videos', 404, blackAdamVideos)
    cy.get('.movie-cover').first().click()
      .get('.exit-movie').click()
      .go(-1).location('pathname').should('include', 'movie')
      .go(1).location('pathname').should('not.include', 'movie')
  })
})